...
#Master user_list
users <- data.frame(user_id = c("1","9","12","16","18","19","21","25","26","30","31","36",
...
for (msnv in mmd_list){
  ...
  for (i in 1:length) {
    ...
    fileConn<-file(paste(a_user, ".seg", sep=""))
    
    seg <- seg_export[mmd_id == msnv,]
    
    segs <- c(msnv, msnv, seg$start, seg$end)

    writeLines(paste(segs, collapse="\t"), sep="", fileConn)
    close(fileConn)
  }
}
