...
for (user in users$user_id) {
  ...
  #fileConn <- file(paste0('adaptive_ref_viz_dynamic_aois_per_user/dynamic_' , user, ".aoi"))
  #fileConn <- file(paste0('control_ref_viz_dynamic_aois_per_user/dynamic_' , user, ".aoi"))
  fileConn <- file(paste0('ml_control_dynamic_aois_per_user/dynamic_' , user, ".aoi"))
  
  dyn_aois <- c()
  for (msnv in msnv_ids) {
    #ref_viz <- fread(paste0('ref_viz_adaptive/ref_viz_', msnv,".aoi"), sep="\n", header = FALSE)
    #ref_viz <- fread(paste0('ref_viz_control/ref_viz_', msnv,".aoi"), sep="\n", header = FALSE)
    ref_viz <- fread(paste0('ml_ref_viz_control/ref_viz_', msnv,".aoi"), sep="\n", header = FALSE)
