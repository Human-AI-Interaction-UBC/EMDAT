...
for (mmd in mmd_list) {
  ...
  fileConn<-file(paste0("ml_ref_viz_control/ref_viz_",mmd, ".aoi"))
  
  if (legend == '') {
    results <- c(Text, Viz, refs_combined, bars_combined, labels)
  } else {
    results <- c(Text, Viz, refs_combined, bars_combined, legend, labels)
  }
