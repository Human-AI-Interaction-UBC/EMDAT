...
from EMDAT_eyetracker.GazepointRecording import GazepointRecording
...
class BasicParticipant(Participant):
    ...
    def __init__(self, pid, eventfile, datafile, fixfile, saccfile, segfile, log_time_offset = None, aoifile = None, prune_length= None,
        ...
        if params.VERBOSE != "QUIET":
            ...
            print "--Eye tracking samples file: "+datafile if datafile is not None else "--No datafile file"
        ...
        elif params.EYETRACKERTYPE == "Gazepoint":
            rec = GazepointRecording(datafile, fixfile, event_file=eventfile, media_offset=params.MEDIA_OFFSET)
        else:
...
def read_participants_Basic(datadir, user_list, pids, prune_length = None, aoifile = None, log_time_offsets=None,
    ...
    for rec,pid,offset in zip(user_list,pids,log_time_offsets):
        ...
        if params.EYETRACKERTYPE == "TobiiV2":
            ...
            allfile = datadir+'/P'+str(rec)+'_all_gaze.csv'
            fixfile = datadir+'/P'+str(rec)+'_fixations.csv'
            evefile = None
            sacfile = None
            segfile = datadir+'/P'+str(rec)+'_TIME.seg'
