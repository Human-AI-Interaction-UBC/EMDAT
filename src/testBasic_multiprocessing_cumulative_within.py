...
os.mkdir('./outputfolder/cumulative_withintask/')

accumulator_window_sizes = [1000, 2000, 5000, 10000]
for window in accumulator_window_sizes:
    for i in range(1, 60000 / window):
        os.mkdir('./outputfolder/cumulative_withintask/window_%d' % window)
        if __name__ == '__main__':
            freeze_support() #for windows
            print("PRUNING %d ms" % (1000 * i))
            ul =        [1, 9] #16, 18, 19, 21, 25, 26, 30,  31,  36, 38, 40, 42, 45, 46, 50,
            ...
            write_features_tsv(ps, './outputfolder/cumulative_withintask/window_%d/pruning_%d.tsv' % (window, i * window), featurelist=params.featurelist, id_prefix=False, require_valid = True)
