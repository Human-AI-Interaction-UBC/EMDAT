...
class BasicParticipant(Participant):
    ...
    def __init__(self, rec, pid, segfile, log_time_offset = None, aoifile = None, prune_length= None,
        ...
        self.features={}
        scenelist, self.numofsegments = partition(segfile, prune_length, disjoint_window, padding, across_tasks, tasks_to_include)
        ...
        print("Finished working on segments and scenes, ", len(self.segments), len(self.scenes))
        if (self.segments == []):
...
def read_participants_Basic(q, datadir, user_list, pids, prune_length = None, aoifile = None, log_time_offsets=None,
                          ...
                          disjoint_window = False, across_tasks = False, time_windows = []):
    ...
    """
    # Recordings in the current process
    recordings_dict = {}
    ...
    rpsdata = read_rest_pupil_sizes(datadir)
                ...
                for pid in pids:
                    ...
                    if os.path.exists(allfile):
                        ...
                        p = BasicParticipant(recordings_dict[pid], pid, recordings_dict[pid].segfile, log_time_offset = offset,
                                            ...
                                            disjoint_window = disjoint_window, padding = window * i, across_tasks = False, tasks_to_include = False)
            ...
            for pid in pids:
                ...
                if os.path.exists(allfile):
                    ...
                    p = BasicParticipant(recordings_dict[pid], pid, recordings_dict[pid].segfile, log_time_offset = offset,
                                        ...
                                        auto_partition_low_quality_segments = auto_partition_low_quality_segments, rpsdata = currpsdata,
                                        disjoint_window = False, padding = 0, across_tasks = across_tasks, tasks_to_include = False)
                    if (p.numofsegments != 0):
                        participants.append(p)
                else:
                    print("Error reading participant files for: %d" % pid)
...
def read_participants_Basic_multiprocessing(nbprocesses, datadir, user_list, pids, prune_length = None, aoifile = None, log_time_offsets = None,
    ...
    if (time_windows == []):
        # There are no windows for cumulative_withintask and across tasks, so we only need
        # to loop once
    ...
    for time_window in time_windows:
            ...
            for i in range(0, nbprocesses):
                if log_time_offsets is None:
    			    p = Process(target=read_participants_Basic, args=(q, datadir, user_listsplit[i], pidssplit[i], prune_length, aoifile, log_time_offsets,
                              require_valid_segs, auto_partition_low_quality_segments, rpsfile, export_pupilinfo, disjoint_window,  across_tasks, [time_window]))
                else:
        ...
        print("Finished the processes, results: ", participants)

        if (disjoint_window):
            for i in range(0, 59001 / time_window):
                cur_participants = []
                for j in range(len(participants)):
                    print(i, j)
                    cur_participants = cur_participants + participants[j][time_window][i]
                if params.VERBOSE != "QUIET":
                    print
                    print "Exporting:\n--General:", params.featurelist
                write_features_tsv(cur_participants, './outputfolder/disjoint_basic/window_%d/chunk_%d.tsv' % (time_window, i), featurelist=params.featurelist, id_prefix=False, require_valid = True)
        elif (not across_tasks):
            for i in range(1, 59001 / 1000):
                cur_participants = []
                for j in range(len(participants)):
                ...
                if params.VERBOSE != "QUIET":
                    print
                    print "Exporting:\n--General:", params.featurelist
                write_features_tsv(cur_participants, './outputfolder/cumulative_withintask_refined_ml/pruning_%d.tsv' % (1000 * i), featurelist=params.featurelist, id_prefix=False, require_valid = True)
        else:
            # Number of tasks
            for i in range(1, 16):
                cur_participants = []
                ...
                if params.VERBOSE != "QUIET":
                    print
                    print "Exporting:\n--General:", params.featurelist
                write_features_tsv(cur_participants, './outputfolder/across_tasks_refined_ml/tasks_included_%d.tsv' % (i), featurelist=params.featurelist, id_prefix=False, require_valid = True)
...
def read_rest_pupil_sizes(datadir):
    pupilsize_dict = {'P21': 3.294234187140609,
    ...
    with open("{dir}/calibration_pupil_sizes_all.tsv".format(dir=datadir), 'rb') as f:
        reader = csv.reader(f, delimiter='\t')
        for row in reader:
            if (len(row) > 2):
                pupilsize_dict['P' + row[0]] = float(row[1])
    return pupilsize_dict
