...
os.mkdir('./outputfolder/disjoint/')
for window in time_windows:
    os.mkdir('./outputfolder/disjoint/window_%d' % window)
    for i in range(60000 / window):
        if __name__ == '__main__':
            freeze_support() #for windows
            print("WINDOW is %d ms" % window)
            print("CHUNK #%d (padding is %d)" %(i, i * window))
            ...
            write_features_tsv(ps, './outputfolder/disjoint/window_%d/chunk_%d.tsv' % (window, i), featurelist=params.featurelist, id_prefix=False, require_valid = True)
