"""
UBC Eye Movement Data Analysis Toolkit (EMDAT)
Created on 11-2019

Sample code showing how to instantiate the "Participant" class for an experiment with a Tobii 4C.

Authors: Martijn Millecamp.
Institution: The University of KU Leuven.
"""
...
from EMDAT_eyetracker.Tobii4CRecording import Tobii4CRecording
    ...
    def __init__(self, pid, eventfile, datafile, fixfile, saccfile, segfile,
        ...
        if params.EYETRACKERTYPE == "TobiiV2":
            ...
            rec = SMIRecording(datafile, fixfile, saccade_file=saccfile, event_file=eventfile,
                               media_offset=params.MEDIA_OFFSET)
        elif params.EYETRACKERTYPE == "Tobii4C":
            rec = Tobii4CRecording(datafile, fixfile, saccade_file=saccfile, media_offset=params.MEDIA_OFFSET)
        elif params.EYETRACKERTYPE == "Tobii4CTest":
            rec = Tobii4CRecording(datafile, fixfile, saccade_file=saccfile, media_offset=params.MEDIA_OFFSET)
        else:
        ...
        print('segments participant' , self.segments)
...
def read_participants_Basic(datadir, user_list, pids, prune_length=None, aoifile=None,
    ...
    for rec, pid, offset in zip(user_list, pids, log_time_offsets):
        ...
        if params.EYETRACKERTYPE == "TobiiV2":
            ...
            allfile = "{dir}/dataTSV/P{rec}_Data_Export.tsv".format(dir=datadir, rec=rec)
            fixfile = "{dir}/fixations/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            sacfile = "{dir}/fixations/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            evefile = "{dir}/fixations/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            segfile = "{dir}/fixations/segments/P{rec}_Segments.seg".format(dir=datadir, rec=rec)

        elif params.EYETRACKERTYPE == "Tobii4CTest":
            allfile = "{dir}/P{rec}_Data_Export.tsv".format(dir=datadir, rec=rec)
            fixfile = "{dir}/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            sacfile = "{dir}/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            evefile = "{dir}/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            segfile = "{dir}/tobii4Ctest.seg".format(dir=datadir, rec=rec)
        else:
            allfile = "{dir}/ETData/c/P{rec}_Data_Export.tsv".format(dir=datadir, rec=rec)
            fixfile = "{dir}/ETData/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            sacfile = "{dir}/ETData/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            evefile = "{dir}/ETData/combined/P{rec}_Data_Fixations.tsv".format(dir=datadir, rec=rec)
            segfile = "{dir}/ETData/segments/P{rec}_Segments.seg".format(dir=datadir, rec=rec)
